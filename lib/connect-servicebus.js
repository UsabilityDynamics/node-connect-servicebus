...
function serviceBus( options ) {
  ...
  process.nextTick( function Configure() {
    ...
    var taget = require( 'url' ).parse( self.options.url || 'amqp://guest:guest@localhost/' );

    if( !taget || !taget.hostname ) {
      return serviceBusMiddleware;
    }

    var connection = amqp.createConnection({
      host: taget.hostname,
      port: taget.port || 8000,
      login: taget.auth ? taget.auth.split( ':' )[0] : '',
      password: taget.auth ? taget.auth.split( ':' )[1] : '',
      clientProperties: {
