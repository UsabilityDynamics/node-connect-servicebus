    ...
    emit: {
      ...
      value: function emit( tag, parent ) {
        ...
        if( tag === 'mount' ) {
          ...
          //parent.emit( 'ping', 'dong' );

          Object.defineProperties( self, {
            publish: {
              value: function publish( tag, data, callback ) {
                console.log( 'publish', arguments );

                // Call Activity
                parent.activity( tag )( data );
          ...
          parent.on( 'cluster.hello', function( data ) {
            console.log( 'cluster.hello', data );

            // Service RPC Work Request.
            self.exchange.publish( 'cluster.hello', data, {
              type: 'message',
              mandatory: false,
              deliveryMode: 1,
              priority: 4,
          ...
          // self.publish =
          // self.subscribe( '')
          // for( var name in parent.engines ) {}
          // console.log( parent.engines );
...
Object.defineProperties( serviceBus.prototype, {
  ...
  publish: {
    ...
    get: function publish() {
      return this.options.get( 'emitter' ) ? this.options.get( 'emitter.emit' ) : process.emit.bind( process );
    },
    enumerable: true,
    configurable: true
  },
  subscribe: {
    ...
    get: function subscribe() {
      return this.options.get( 'emitter' ) ? this.options.get( 'emitter.on' ) : process.on.bind( process )
    },
    enumerable: true,
    configurable: true
  },
              ...
              function requestCallback( error, message ) {
                ...
                self.exchange.publish([ self.name, 'cid', self.correlationId ].join( '.' ), message || error || {}, {
                  ...
                  headers: {
                    'correlationId': requestCallback.correlationId || '',
                    'product': self.options.get( 'product' ),
                    'x-host': self.options.get( 'version' ),
                    'x-pid': process.pid
                  }
                });
